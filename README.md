...
[下载最新版 ForCon](https://github.com/MartinG031/ForCon/releases/latest/download/ForCon-0.1.33.dmg)
...
4. 首次打开时，如果 macOS 提示安全确认，请打开“系统设置 > 隐私与安全”，点击 ForCon 的“仍要打开”；也可以右键 `ForCon.app`，选择“打开”。
...
ForCon 启动时会自动检查更新。需要安装新版时，点击 macOS 顶部菜单栏的“ForCon > 检查更新...”。ForCon 会：
...
- 输出文件只写入用户选择的输出目录。
- 只有启动检查更新或通过 ForCon 菜单安装更新时才会访问 GitHub Releases。
- 下载的更新包会先做 SHA-256 校验。
- 当前发行包为本地 ad-hoc 签名，尚未使用 Apple Developer ID 公证。
- 每个 GitHub Release 附带 DMG、ZIP 和 `latest.json`，`latest.json` 包含版本、下载地址和 SHA-256。
...
可以在“设置 > 转换组件”中查看这些工具是否已安装；缺失时按提示安装对应组件。
...
[Download the latest ForCon release](https://github.com/MartinG031/ForCon/releases/latest/download/ForCon-0.1.33.dmg)
...
4. On first launch, if macOS shows a security prompt, open System Settings > Privacy & Security and click Open Anyway for ForCon. You can also right-click `ForCon.app` and choose Open.
...
ForCon checks for updates on launch. To install a new version, click ForCon > Check For Updates... in the macOS menu bar. ForCon will:
...
- GitHub Releases is contacted only when ForCon checks for updates on launch or installs an update from the ForCon menu.
...
- Each GitHub Release includes a DMG, ZIP, and `latest.json`; `latest.json` contains the version, download URL, and SHA-256.
...
Open Settings > Conversion Components to check whether these tools are installed. If a tool is missing, follow the install hint shown there.
